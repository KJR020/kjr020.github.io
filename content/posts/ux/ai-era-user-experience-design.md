...
ここまでUXとUXデザインについて整理してきました。ここから、ギャレットの5層モデルを見ていきます。
...
> [!note] 「段階」と「レイヤー」の表記
> 日本語版では、このモデルを「5段階モデル」と呼び、各 *plane* も「戦略段階」のように訳しています。この記事では「5層モデル」と呼び、各 *plane* は「レイヤー」と表記します。「段階」という言葉から、順番に進める設計工程だと受け取られるのを避けるためです。この点は、後の「5つのレイヤーは設計工程ではない」で改めて触れます。
