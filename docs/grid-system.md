...
| Wide | 記事ヘッダーは12 / 12 columns、その下の本文は9 / 12 columns、目次を閉じると拡張 | 3 / 12 columns、閉じたときは再表示操作のみ |

Wideでは記事ヘッダーを全幅に置き、その下に本文と目次を並べて1 gutterを空ける。目次を閉じると再表示操作に必要な幅だけをRailに残し、本文を空いた領域へ広げる。本文内は外側のPage Gridとは独立した9 tracksで構成し、本文・見出し・リストを8 / 9、Figure・Code・Tableを9 / 9へ配置する。Compactでは両方を1 columnへ戻す。CompactとMediumの目次は、記事ヘッダーの直後へインラインに配置する。
