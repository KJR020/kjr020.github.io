# KJR020ブログ Grid system

Atlassian Design Systemの考え方をKJR020ブログへ適用した、ページ横方向の正規配置ルール。
...
KJR020ブログでは、その原則を次のように調整する。

- Atlassianの6 breakpointを、KJR020ブログの`md: 768px`と`lg: 1024px`へ合わせて3段階に圧縮する。
...
- FluidはKnowledge Graphなど、横方向の拡張に意味がある領域の内部に限定する。
...
| Type | 最大幅 | 用途 | 主なページ |
| --- | ---: | --- | --- |
| Fixed-wide | 1152px | 本文と補助領域を横に並べる構造化ページ | 記事＋目次、検索＋タグ、Knowledge |
| Fixed-narrow | 864px | 読むことが主目的の長文ページ | 記事単体、Privacy Policy、説明文書 |
| Fluid | なし | 横方向の広がり自体に意味がある領域 | Knowledge Graph、横scroll carouselの内部 |
...
Wideでは記事と目次の間に1 gutterを置く。Articleの子要素は親の9 columnsへ再度揃えず、本文用spacingと読み幅で構成する。
...
- [Living catalog](../src/design-system/pages/index.astro) - Gridの視覚例（`pnpm dev`の`/design-system`）
