...
KJR020ブログのUIは、記事を主役にしながら操作と状態を分かりやすく伝える。
...
- Navigationのカテゴリ名とサービス固有名は英語を使用できる。
  - `Home`, `Posts`, `Knowledge`, `Search`, `Scrapbox`
- 説明、操作、状態は日本語を基本とする。
- 技術固有名詞は公式表記を使う。
...
- 件数は対象と隣接させる。
  - `12件の記事`
  - `3件のタグ`
- 相対日付はKnowledge内に限定し、記事公開日には使わない。
...
| Page title | 場所を示す名詞 | `Knowledge Graph` |
...
| Knowledgeエラー | `Knowledge Graphを読み込めませんでした` | 操作: `再読み込み` |
...
ルールを変更する場合は、変更理由を合意したうえでこの文書とLiving catalogを同時に更新する。実装とテストはこの仕様へ適合させる。
...
- [Living catalog](../src/design-system/pages/index.astro) - UIライティングの視覚サマリー（`pnpm dev`の`/design-system`）
...
- [KnowledgeGraph.tsx](../src/components/knowledge/KnowledgeGraph.tsx) - Knowledge Graphの状態表示
