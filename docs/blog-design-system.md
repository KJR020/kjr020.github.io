...
| Article variant | 記事タイトルを文字量に応じて42〜68pxのφ¼ステップで調整し、最大3行を基準に表示する。書名などの引用部分は途中で分断せず、前後の意味単位で改行する |
...
| Preview | 開発サーバー限定の`/og-preview`で共通・記事別画像を並べ、タイトルと主要な余白を調整する。本番成果物には含めない |
